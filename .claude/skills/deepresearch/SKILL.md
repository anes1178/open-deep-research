...
description: |
  ...
  호출 방식:
  - `/deepresearch plan "질의"` — 요구사항 분석 후 MD 저장, 사용자 확인 대기
  - `/deepresearch do` — 저장된 MD 기반으로 코드 생성
  - `/deepresearch "질의"` — plan + do 전체 파이프라인 자동 실행 (확인 없음)

  Examples:
  - `/deepresearch plan "A 사이트를 참고해서 FastAPI 서버 만들어줘"`
  - `/deepresearch do`
  - `/deepresearch "https://example.com 보고 인증 API 만들어줘"`
...
> `/require` + `/implement` 패턴과 동일하지만, 리서치 단계에서 MCP(Confluence, GitHub)를 활용합니다.
...
- 첫 단어가 `plan` → **Plan 모드** (Step 1~3만 실행, Step 4에서 대기)
- 첫 단어가 `do` → **Do 모드** (Step 4만 실행)
- 그 외 또는 인자 없음 → **Full 모드** (Step 1~4 자동 실행, 확인 없음)

---

## Step 1: 입력 파싱 (Plan 모드 / Full 모드)

- `plan` 키워드 이후의 텍스트를 질의(query)로 사용합니다.
- Full 모드에서는 인자 전체를 질의로 사용합니다.
- 질의에서 URL이 있는지 확인합니다 (http:// 또는 https:// 포함 여부).
- 질의가 너무 짧거나 모호하면(5단어 미만, URL 없음) AskUserQuestion으로 목표를 구체화합니다.

---

## Step 2: 요구사항 리서치 실행 (Plan 모드 / Full 모드)
...
4. 각 fetch 후 반드시 think_tool로 다음을 판단합니다:
   - 추출한 기술 정보는 무엇인가?
   - 어떤 링크를 따라갈 것인가, 이유는?
   - 이미 충분한 스펙이 확보됐는가?
...
**Plan 모드**: 저장 후 사용자에게 다음을 안내하고 대기합니다:
> "요구사항 파일을 저장했습니다: `requirements/{feature-name}.md`
> 파일을 검토·수정한 뒤 `/deepresearch do`를 실행하면 코드를 생성합니다."
> → 여기서 종료합니다.
...
- 파일이 없으면: "생성된 요구사항 파일이 없습니다. 먼저 `/deepresearch plan`을 실행하세요." 안내 후 종료.
